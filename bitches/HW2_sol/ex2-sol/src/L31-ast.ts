...
;;         |  ( class ( <var>+ ) ( <binding>+ ) ClassExp(args:VarDecl[],bindings:Binding[])) //L31
...
const parseClassExp = (varDecls: Sexp, bindings: Sexp): Result<ClassExp> => {
    ...
    const vars = map(b => b[0], bindings);
    const vals : Result<CExp[]> = mapResult(binding => parseL31CExp(second(binding)), bindings);
...
const unparseClassExp = (ce: ClassExp) : string => 
    `(class (${map((f: VarDecl) => f.var, ce.fields).join(" ")}) (${map((b: Binding) => `(${b.var.var} ${unparseL31(b.val)})`, ce.methods).join(" ")}))`
