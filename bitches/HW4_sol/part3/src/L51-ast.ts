...
import { allT, first, cons, rest, second, isEmpty } from '../shared/list';
...
export type CompoundExp = AppExp | IfExp | ProcExp | LetExp | LitExp | LetrecExp | SetExp | ClassExp;
export const isCompoundExp = (x: any): x is CompoundExp =>
    isAppExp(x) || isIfExp(x) || isProcExp(x) || isLitExp(x) || isLetExp(x) || isLetrecExp(x) || isSetExp(x) || isClassExp(x);
export const expComponents = (e: Exp): CExp[] =>
    ...
    isSetExp(e) ? [e.val] :
    isDefineExp(e) ? [e.val] :
    isClassExp(e) ? map((b) => b.val, e.methods) : 
    isAtomicExp(e) ? [] :
    isLitExp(e) ? [] :
    e; 
...
const parseGoodClassExp = (typeName: Sexp, varDecls: Sexp, bindings: Sexp): Result<ClassExp> =>
    ! isString(typeName) ? makeFailure(`class first param must be a type var - got ${typeName}`) :
    isToken(varDecls) ? makeFailure(`class: expected a list of fields - got ${varDecls}}`) :
    ! isGoodBindings(bindings) ? makeFailure(`class: expected method bindings - got ${bindings}`) :
    bind(mapResult(parseVarDecl, varDecls), 
         (fields: VarDecl[]) => 
           bind(parseBindings(bindings),
                (bindings: Binding[]) => makeOk(makeClassExp(makeTVar(typeName), fields, bindings))));
...
export const parsedToClassExps = (p: Parsed): ClassExp[] => 
    ...
    isAtomicExp(p) ? [] :
    isLitExp(p) ? [] :
    isAppExp(p) ? chain(parsedToClassExps, [p.rator, ...p.rands]) :
    isProcExp(p) ? chain(parsedToClassExps, p.body) : 
    isLetExp(p) ? chain(parsedToClassExps, concat(map((b: Binding) => b.val, p.bindings), p.body)) :
    isLetrecExp(p) ? chain(parsedToClassExps, concat(map((b: Binding) => b.val, p.bindings), p.body)) :
    isSetExp(p) ? parsedToClassExps(p.val) :
    isIfExp(p) ? chain(parsedToClassExps, [p.test, p.then, p.alt]) :
    isClassExp(p) ? cons(p, chain(parsedToClassExps, map((b: Binding) => b.val, p.methods))) :
    p;
