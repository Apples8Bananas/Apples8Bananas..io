...
export const makeTEnvFromClasses = (parsed: A.Parsed): E.TEnv => {
    ...
    const classTExps = R.map(A.classExpToClassTExp, classes);
    return isEmpty(classes) ? E.makeEmptyTEnv() : E.makeExtendTEnv(R.map(c => c.typename, classTExps), classTExps, E.makeEmptyTEnv());
}
...
export const inferTypeOf = (concreteExp: string): Result<string> =>
    bind(A.parse(concreteExp), 
         parsed => {
            const tenv =  makeTEnvFromClasses(parsed);
...
export const typeofDefine = (exp: A.DefineExp, tenv: E.TEnv): Result<T.VoidTExp> => {
    const constraint = bind(typeofExp(exp.val, E.makeExtendTEnv([exp.var.var], [exp.var.texp], tenv)), 
                            (valTE: T.TExp) => checkEqualType(exp.var.texp, valTE, exp));
...
// Typing rule program:
//   For every: type environment _Tenv
//              program Program([_exp1])
//              type expression _E1
//   if _Tenv |- _exp1 : _E1
//   then _Tenv |- _p: _E1
// 
//   For every: type environment _Tenv
//              program _p = Program([_def1, _e2, ..., _en])
//              define expression _def1 = (define _v1 _e1)
//              type expressions _E1, _P
//   if _Tenv o {_v1: _E1} |- e1: E1 and
//      _Tenv o {_v1: _E1} |- Program(_e2, ..., _en): _P
//   then _Tenv |- _p: _P
//   
//   For every: type environment _Tenv
//              program _p = Program([_e1, _e2, ..., _en]), n > 1
//              _e1 is a CExp (not a defineExp)
//              type expression _E1, _P
//   if _Tenv |- e1: E1 and
//      _Tenv |- Program(_e2, ..., _en): _P
//   then _Tenv |- _p: _P
...
export const typeofLit = (exp: A.LitExp): Result<T.TExp> =>
    isNumber(exp.val) ?  makeOk(T.makeNumTExp()) :
    exp.val === true ? makeOk(T.makeBoolTExp()) :
    exp.val === false ? makeOk(T.makeBoolTExp()) :
    isString(exp.val) ? makeOk(T.makeStrTExp()) :
    V.isSymbolSExp(exp.val) ? makeOk(T.makeSymbolTExp(exp.val)) :
    V.isEmptySExp(exp.val) ? makeOk(T.makeSymbolTExp()) :
...
export const typeofSet = (exp: A.SetExp, tenv: E.TEnv): Result<T.VoidTExp> => {
    const constraint = bind(typeofExp(exp.val, tenv), 
                            (valTE: T.TExp) => bind(E.applyTEnv(tenv, exp.var.var),
                                                    (varTE: T.TExp) => checkEqualType(varTE, valTE, exp)));
    return bind(constraint, _ => makeOk(T.makeVoidTExp()));
};
...
export const typeofClass = (exp: A.ClassExp, tenv: E.TEnv): Result<T.TExp> => {
    const fields = R.map((f) => f.var, exp.fields);
    const fieldTEs = R.map((f) => f.texp, exp.fields);
    const methodTEs = R.map((m) => m.var.texp, exp.methods);
    const methodProcs = R.map((m) => m.val, exp.methods);
    const tenvClass = E.makeExtendTEnv(fields, fieldTEs, tenv);
    const constraints = zipWithResult((methodTE, methodProc) => bind(typeofExp(methodProc, tenvClass),
                                                                     (methodCTE: T.TExp) => checkEqualType(methodTE, methodCTE, exp)),
                                      methodTEs, methodProcs);
    const classTypeConstraint = bind(constraints, _ => checkEqualType(exp.typeName, A.classExpToClassTExp(exp), exp));
    return bind(classTypeConstraint, _ => makeOk(T.makeProcTExp(fieldTEs, exp.typeName)));
};
